...
2.  Add a `FieldConfig` object to the relevant list (e.g., `RELEASE_FIELDS`).
    ```python
    FieldConfig(
        agol_field="new_field",
        sf_field="Salesforce_Field__c",
        alias="New Field Alias",
        field_type=FieldConfig.text
    )
    ```
...
- **Mode Selection**: In `main.py`, the `process()` function calls `skid.publish()` or `skid.update()`.
  - Default is often `publish()` for setup. Change to `update()` for routine runs.
...
- **Error Handling**: `supervisor` catches errors in production and sends emails.
